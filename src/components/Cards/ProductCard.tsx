...
const ProductMedia: React.FC<{ thumbnail: any, heading: string;}> = ({ thumbnail, heading }) => (
    <div className='min-w-[100px] lg:min-w-[120px] py-5 md:py-0'>
    {thumbnail && <div className='w-[80px] min-w-[80px] h-[80px] lg:w-[115px] lg:h-[115px] lg:min-w-[115px] border-[#C1DED5] mb-2 md:mb-3 rounded-full flex items-center justify-center p-[3px] md:p-[6px] mx-auto border-dashed border-2'>
...
const ProductAction: React.FC<{ price: number; button_label: string, button_url: string}> = ({ price, button_label, button_url }) => (
        ...
        {product.banner && <ProductMedia heading={product.name} thumbnail={product.banner} />}
         ...
         <div  className='block md:hidden relative before:content-[""] before:w-[1px] before:h-[calc(100%-2rem)] before:bg-border before:absolute before:start-0 before:top-1/2 before:-translate-y-1/2 md:before:content-[unset]'>
            ...
            {product.price_from && <div className='mt-auto md:border-t md:border-border md:pt-2'>
                <ProductAction price={product.price_from}  button_label={'Buy Now'}  button_url={'#'} />
                </div>}
         </div>
